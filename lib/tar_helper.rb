module TarHelper
  ...
  def self.untar(args, file, base_dir = nil)
    ...
    result = `tar #{args} "#{SecurityHelper.sanitize_file_path(file)}" #{base_dir_str}; echo $?`
    result.to_i == 0
  end
  ...
  def self.tar(args, file, files, base_dir = nil, exclude_files = nil)
    ...
    result = `tar #{args} "#{SecurityHelper.sanitize_file_path(file)}" #{base_dir_str} #{file_str} #{exclude_file_str}; echo $?`
    result.to_i == 0
  end
