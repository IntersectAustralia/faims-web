...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
  ...
  <xsl:template match="/">
    ...
    <xsl:for-each select="dataSchema/RelationshipElement">
      INSERT INTO RelnType(RelnTypeID, RelnTypeName, RelnTypeDescription, RelnTypeCategory, Parent, Child) VALUES('<xsl:value-of select="substring(generate-id(.),4)"/>', '<xsl:value-of select="@name"/>', '<xsl:value-of select="description"/>','<xsl:value-of select="@type"/>', '<xsl:value-of select="parent"/>', '<xsl:value-of select="child"/>');
